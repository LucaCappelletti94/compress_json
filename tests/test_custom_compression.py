...
def test_custom_compression():
    ...
    compress_json.dump(
        D, "filepath.custom_extension1", compression="gzip"
    )  # for a gzip file
    compress_json.dump(
        D, "filepath.custom_extension2", compression="bz2"
    )  # for a bz2 file
    compress_json.dump(
        D, "filepath.custom_extension3", compression="lzma"
    )  # for a lzma file
    ...
    assert (
        compress_json.local_load("filepath.custom_extension1", compression="gzip") == D
    )
    assert (
        compress_json.local_load("filepath.custom_extension2", compression="bz2") == D
    )
    assert (
        compress_json.local_load("filepath.custom_extension3", compression="lzma") == D
    )
